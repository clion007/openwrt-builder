...
export Op_name="ClionRt"                    # 修改主机名称为OpenWrt-123(填0为不作修改)

# 内核和系统分区大小(不是每个机型都可用)
export Kernel_partition_size="0"            # 内核分区大小,每个机型默认值不一样 (填写您想要的数值,默认一般16,数值以MB计算，填0为不作修改),如果你不懂就填0
export Rootfs_partition_size="220"          # 系统分区大小,每个机型默认值不一样 (填写您想要的数值,默认一般300左右,数值以MB计算，填0为不作修改，默认最小180),如果你不懂就填0
...
sed -i 's/"UPnP IGD 和 PCP"/"UPnP服务"/g' `egrep "UPnP IGD 和 PCP" -rl ./`
sed -i 's/"Tcpdump 流量监控"/"数据捕获"/g' `egrep "Tcpdump 流量监控" -rl ./`
sed -i 's/"Vlmcsd KMS 服务器"/"KMS服务"/g' `egrep "Vlmcsd KMS 服务器" -rl ./`
...
echo 'KEEP_CONF_FILES="/etc/dnsmasq.d/ /usr/share/adss/ /etc/init.d/adss /etc/rc.d/S18adss /etc/puship/ /etc/hotplug.d/iface/99-puship"' >> ${GITHUB_ENV}
